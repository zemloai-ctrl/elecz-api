...
User asks about contracts, savings potential or switching provider → `best_energy_contract` (MCP) or `GET /signal/contract` (REST)

## Important Market Notes
Some markets (AU, NZ, South Korea, South Africa, Philippines) don't have public day-ahead data → `cheapest_hours` returns `available: false`
Mexico: Day-ahead available via CENACE (14 zones) — `cheapest_hours` works, but wholesale only — retail via CFE includes distribution and subsidies
USA & Canada: Only wholesale prices available (retail prices are higher)
Japan: Day-ahead available, but no contract comparison
South Africa (ZA): Eskom regulated tariff in ZAR c/kWh (VAT excl) — `price_type: regulated`, fixed until 1 April each year
Philippines (PH-LUZ/VIS/MIN): Regulated distribution tariffs in PHP c/kWh — updated monthly, no contract comparison
